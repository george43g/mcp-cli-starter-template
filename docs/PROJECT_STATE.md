...
| Registry state | Published: **`@george43g/robustness@0.5.1`**, **`@george43g/cli-kit@0.2.1`**, **`@george43g/tui-kit@0.3.1`**, **`@george43g/secret-store@0.2.1`** (all CI OIDC, 2026-08-09). The `.1` patches are the CJS `exports` fix (PR #22) — verified by `require()`-ing the published tarballs, root and subpath. The minors before them are 16a: robustness 0.5.0 (redaction, logger knobs, shutdown default sink, `exitOnUnhandledRejection`) and tui-kit 0.3.0 (`useDevStats(visible)`). Earlier trail from PR #19: robustness 0.4.0, cli-kit 0.2.0, tui-kit 0.2.0, secret-store 0.2.0. Trail note: robustness 0.3.0 and secret-store 0.2.0 were ACCIDENTAL — a `feat(vitest-config):` commit touched files inside their directories and `semantic-release-monorepo` reads the type against paths, not the scope (field-note 52); release trigger paths now exclude test/tooling files. Earlier trail: Trail: robustness 0.1.0 was the 2026-07-29 user-run bootstrap, 0.1.1 the first CI OIDC release (2026-07-31), 0.2.0 cut by CI 2026-08-09, 0.2.1 (DEFERRED #14) cut by CI the same day; the kits' 0.1.0 were user-run bootstraps (2026-08-08, tagged at `cb21bea`) and tui-kit 0.1.1 was cut by CI. Trusted Publishers configured for all three — CI releases from here with no `NPM_TOKEN`. Build provenance removed: it needs a public source repo and would 422 (field-note 23 supersedes 19) |
