...
The starter ships a working Ink/React TUI that demos the patterns. Tools that don't need a TUI can delete `apps/example-repo-mcp/src/tui/` and the `tui` subcommand of the `example-repo` bin; everything else still works.
...
2. Remove the `tui` subcommand from `apps/example-repo-mcp/src/cli.ts`. (There is no separate TUI bin to remove: the app ships one bin, `example-repo`.)
3. Remove the TUI entry from `apps/example-repo-mcp/vite.config.ts` `lib.entry`.
4. Optionally remove `@george43g/tui-kit` from `apps/example-repo-mcp/package.json` dependencies.
