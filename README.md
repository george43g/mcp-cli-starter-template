...
| 10-docs-readme | Mintlify config + MDX scaffold + reference markdown + public-style README + the guard scripts generated CI invokes (`check-docs-links`, `check-stdout-purity`, `check-release-tokens`, and `mcp-apps` + `for-each-mcp-app`, which select the gated apps by their `@george43g/mcp-kit` dependency rather than by name, and skip with a notice when a repo has no MCP app) |
