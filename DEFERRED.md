...
| `robustness` minor (`WatchdogOptions.onBreach`, PR #59) | Give the HTTP path watchdog **detection without the kill**: `installWatchdog({ onBreach: () => "observe" })` in `runHttpMcp` (`apps/example-repo-mcp/src/commands/http.ts`), plus the `08-app/lib/` mirror and `pnpm regen:example`. Parked, not forgotten: `onBreach` does not exist in `robustness@0.8.1`, so the call site typechecks here (pnpm links the workspace copy) and fails the scaffolder E2E smoke with `TS2305` — the TWO-PRs rule exactly. Confirm the shape before writing it: the approved scope for the HTTP path was graceful shutdown only, with the watchdog **not** enabled by default, and observe-only detection is a third position between "absent" and "self-killing" that nobody has explicitly signed off. | ⏳ waiting on the release that ships `onBreach` |
