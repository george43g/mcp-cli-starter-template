...
`apps/example-repo-mcp/src/commands/http.ts`. `MCP_HTTP_TOKEN` in the
...
- **No TUI**: delete `apps/example-repo-mcp/src/tui/`, the `tui` subcommand from `src/cli.ts` (there is one bin, `example-repo`; the TUI is a subcommand of it, not a bin of its own), and the TUI entry from `vite.config.ts`. Optionally drop `@george43g/tui-kit` from the app's `dependencies`.
