...
|---|---|---|
| MCP server | `example-repo mcp` (stdio; `--http` for Streamable HTTP) | wired, tested |
| TUI | `example-repo tui` | wired, tested |
| REPL | `example-repo repl` (alias `console`) | wired, tested |
| Direct tools | `example-repo health`, `example-repo noop`, … | one CLI subcommand per tool |
...
- **One bin, four surfaces.** A single `example-repo` binary multiplexes the MCP
...
<!-- endif:rust-accel -->
<!-- if:!rust-accel -->
- **No native acceleration.** Generated with `--no-rust-accel`, so there is no
  `apps/rust-accel/`; [RUST_ACCELERATION.md](RUST_ACCELERATION.md) covers adding
  it later.
<!-- endif:!rust-accel -->
...
3. **Rename if needed.** The bin/scope are already `example-repo` / `@george43g`;
