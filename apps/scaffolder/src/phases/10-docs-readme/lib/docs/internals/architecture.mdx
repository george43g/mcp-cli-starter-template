...
```
{{name}} mcp   → runMcpServer({ transport: 'stdio' | 'http' })
{{name}} tui   → dynamic import of src/tui/index.tsx → runTui()
{{name}} repl  → readline loop → program.parseAsync per line
{{name}} <tool> → callMcpTool(toolName, argsFromFlags)
```
