...
A Turborepo monorepo that ships four surfaces from one `example-repo` bin: an MCP
...
example-repo mcp   # run the built MCP server over stdio
```

- Explore a surface: `example-repo tui`, `example-repo repl`, or a direct tool
  like `example-repo health`.
