...
![example TUI](docs/screenshots/tui.gif)
![example CLI](docs/screenshots/overview.gif) -->
...
`@george43g/example` is not on npm until you publish it
([docs/RELEASE.md](docs/RELEASE.md)). Until then, install from a clone:

```bash
pnpm install && pnpm build
cd apps/example && pnpm link --global   # puts `example` on your PATH
```

Once published:

```bash
npx @george43g/example mcp              # run without installing
...
```bash
example mcp              # run the MCP server (stdio)
example mcp --http       # run via Streamable HTTP (requires MCP_HTTP_TOKEN)
example tui              # launch the Ink TUI
example doctor           # preflight checks (Node version, native deps, env)
example repl             # interactive REPL — same dispatcher as MCP
example health           # one-shot health snapshot
example noop --input hi  # call any tool directly from the CLI
```
...
`"command": "node", "args": ["/absolute/path/to/apps/example/dist/cli.js", "mcp"]`
(or `"command": "example", "args": ["mcp"]` after `pnpm link --global`).
...
| `get_logs` | **Dev-mode only** (`EXAMPLE_DEV=1`). Returns the last N lines from the in-memory ring buffer. | read-only |

## The companion skill

This repo ships an agent skill that teaches an AI agent how to use your tool end-to-end. It lives at `.agents/skills/example/SKILL.md` and is meant to be rewritten by you (or by the AI itself, after first reading the tool) to document the tool's actual behavior.

Inside this repo every agent already sees it: Codex, opencode and Cursor read `.agents/skills/`, and Claude Code reads `.claude/skills/example`, a tracked relative symlink to the same directory. To use it from other repos too, link it into your user skills directory:

```bash
ln -s "$(pwd)/.agents/skills/example" ~/.claude/skills/example
ln -s "$(pwd)/.agents/skills/example" ~/.agents/skills/example
```
...
.agents/skills/example/             Repo-installable companion skill (rewrite at scaffold time)
```
