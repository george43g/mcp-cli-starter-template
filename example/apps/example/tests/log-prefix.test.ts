...
async function runIsolated(sub: string): Promise<string> {
  ...
  // os.tmpdir() reads TMPDIR on POSIX but TEMP, then TMP, on win32. Redirecting
  // only TMPDIR sends a Windows child's logs to the real temp dir while this
  // test inspects an empty sandbox, so every case passes vacuously there.
  const env = {
    ...process.env,
    TMPDIR: sandbox,
    TEMP: sandbox,
