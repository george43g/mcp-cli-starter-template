...
if ! type -p usage &> /dev/null
    echo >&2
    echo "Error: usage CLI not found. This is required for completions to work in example." >&2
    echo "See https://usage.jdx.dev for more information." >&2
    return 1
end

set _usage_spec_example 'name example
bin example
about "example — MCP server + CLI + TUI (single bin, subcommand dispatch)"
...
set -l spec_file "$tmpdir/usage__usage_spec_example.spec"
echo $_usage_spec_example > "$spec_file"

set -l tokens
if commandline -x >/dev/null 2>&1
    complete -xc example -a "(command usage complete-word --shell fish -f \"$spec_file\" -- (commandline -xpc) (commandline -t))"
else
    complete -xc example -a "(command usage complete-word --shell fish -f \"$spec_file\" -- (commandline -opc) (commandline -t))"
end
