...
A Turborepo monorepo that ships **four surfaces** from a **single bin** (`example`):

| Subcommand | Surface |
|---|---|
| `example mcp` | MCP server (stdio default; `--http` for Streamable HTTP) |
| `example tui` | Ink/React full-screen TUI |
| `example doctor` | Preflight checks (Node version, native module, env) |
| `example repl` (alias `console`) | Interactive REPL driving the in-process dispatcher |
| `example health`, `example noop`, … | Direct tool invocation — one CLI subcommand per `ToolDefinition` |
...
Default off (stdio mode). Enable with `example mcp --http`. Requires `MCP_HTTP_TOKEN` (generate with `openssl rand -hex 32`).
